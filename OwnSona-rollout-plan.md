...
| Learning memory Tier 3 — consolidation ("sleep") job + generative seam (`OwnSona.md` §3.1) | **code-complete** | **Procedure A (code-only, no migration, no version bump).** New **optional** generative-LLM seam `ai.ownsona.llm.GenerativeProvider` (+ `OpenAIGenerativeProvider` / `MockGenerativeProvider`), configured **separately** from embeddings via `LLM_API_KEY` / `LLM_MODEL` / `LLM_ENDPOINT` / `LLM_PROVIDER`. **Invariant #1 relaxed**: generative calls are now allowed via this seam for background/maintenance work, but never on the synchronous recall/remember hot path; with no `LLM_API_KEY` the server runs exactly as before. `ConsolidationJob` (driven by Kiss Cron via `backend/CronTasks/Consolidate.groovy`) clusters near-duplicates (`find_near_duplicates`, threshold default 0.95), asks the LLM to merge each into one canonical fact, stores it (`dedup_policy=insert`), and **supersedes** the originals (recoverable soft-delete + `replaced_by_id`). **Off by default 3 ways** (crontab line commented, `CONSOLIDATION_ENABLED=false`, no-op without LLM). Skips any cluster containing a `keep='Y'` member. **Cost-bounded**: ≤ `CONSOLIDATION_MAX_GROUPS` (default 25) small LLM calls/run, zero when no clusters exist. Merges + summary logged at WARN for audit/undo. Rollback: redeploy previous WAR (and re-comment the crontab line). |
