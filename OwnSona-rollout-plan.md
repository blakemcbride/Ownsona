...
| 4 — Dedup + freshness | **deployed** (2026-05-16) | `CURRENT_DB_VERSION` bumped to 3 via `Migration003AddFreshness` (adds `expires_at`, `last_confirmed_at` with partial index). Semantic dedup-on-write check (0.90 threshold, top-K=5) with three-way `dedup_policy` (`insert` / `skip_if_near` / `ask`, default `ask`). New `confirm` MCP tool. Expired rows excluded from recall / list_memories / text_search; `include_deleted=true` bypasses for diagnostics |
| 5 — Conflict surfacing + tombstones (5C decay skipped) | **deployed** (2026-05-16) | 5A: `recall` tool description now tells the calling LLM to prefer the most recently confirmed match when multiple hits look contradictory (no schema, no code paths). 5B: `CURRENT_DB_VERSION` bumped to 4 via `Migration004AddTombstones` (adds `forget_reason TEXT` and `replaced_by_id BIGINT REFERENCES memories(id)`); `forget` MCP tool gains optional `reason` + `replaced_by_id` (rejected on `hard_delete=true`); dedup-on-write check now also surfaces tombstone near-dups as a separate `previously_corrected` array. 5C (soft decay) deliberately skipped. |
| 6 — Embedding-model migration framework | **code-complete** | `ReembedJob` walker re-embeds rows whose `(embedding_provider, embedding_model)` doesn't match active config or whose embedding is NULL; runs from `MCPServer.<clinit>` after `DbMigrator`/`RecordMigrator` when `REEMBED_ON_STARTUP=true`. `ApplicationIniWriter` flips the flag back to `false` after a clean run. Same-dim switches need no migration; different-dim switches ship an additive migration that resizes the column and the walker fills it. Full operator procedure in `REEMBED.md`. No `CURRENT_DB_VERSION` bump (the framework is code-only; the first real dim-change deploy will ship its own migration). |
| Learning memory Tier 1 — salience + reinforcement + conflict handling (`OwnSona.md`, no-decay variant) | **code-complete** | Procedure B: `CURRENT_DB_VERSION` → **6** via `Migration006AddSalience` (adds `salience`, `use_count`, `reward_sum`, `last_used_at`); `CURRENT_RECORD_VERSION` → **2** via `SalienceSeedUpgrader` (seeds `salience` from `importance`, idempotent). New `reinforce` MCP tool, and `confirm` now also reinforces; reward rule `salience ← clamp((1−λ)·salience + η·reward)`, η=0.3 λ=0.05 clamp [0,3]. `recall` ranks by `cosine·(1 + 0.5·salience)` with recency tiebreaker — `score` stays cosine (so `min_score` unchanged). New `find_conflicts` tool (tag-gated, default 0.80) + `potential_conflicts` array on `remember`; `remember(supersedes=[…])` auto-supersedes named ids (soft-delete + `replaced_by_id`), `keep='Y'` rows reported `protected`. **Deliberately no time-decay and no age-based pruning** (diverges from roadmap §1.2 per product decision). Rollback SQL: `ALTER TABLE memories DROP COLUMN salience, DROP COLUMN use_count, DROP COLUMN reward_sum, DROP COLUMN last_used_at;` (and the row stays at record_version 2 harmlessly). |
| Learning memory Tier 2 — non-destructive supersession (`OwnSona.md` §2.1, no-decay variant) | **code-complete** | **Procedure A (code-only, no migration, no version bump).** Adds the second conflict-resolution lever to `remember`: `downweights=[ids]` applies a strong negative reinforcement (`CONFLICT_PENALTY = −1.0`) to outdated-but-not-wrong memories so the new fact out-ranks them while they stay active and recallable — the "un-learn the stale answer" behavior, adapted to the no-delete rule (`supersedes` remains the destructive path for facts that are actually wrong). Both `remember` levers respect `keep='Y'` (report `protected`); raw `reinforce` stays exempt. Tool descriptions (`remember`, `find_conflicts`) now guide the in-loop client LLM to close the loop: surface conflict → judge → `supersedes` (wrong) / `downweights` (outdated) / leave. Rollback: redeploy the previous WAR. |
| Learning memory Tier 3 — maintenance ("sleep") job (merge/dedup + conflict-resolution) + generative seam (`OwnSona.md` §3.1) | **code-complete** | **Procedure A (code-only, no migration, no version bump).** New **optional** generative-LLM seam `ai.ownsona.llm.GenerativeProvider` (+ `OpenAIGenerativeProvider` / `MockGenerativeProvider`), configured **separately** from embeddings via `LLM_API_KEY` / `LLM_MODEL` / `LLM_ENDPOINT` / `LLM_PROVIDER`. **Invariant #1 relaxed**: generative calls allowed via this seam for background/maintenance work, never on the synchronous recall/remember hot path; with no `LLM_API_KEY` the server runs exactly as before. `ConsolidationJob` (Kiss Cron via `backend/CronTasks/Consolidate.groovy`) runs two independently-gated passes: **(1) consolidation/dedup** (`CONSOLIDATION_ENABLED`, threshold 0.95) — merge near-identical clusters into one canonical fact (`dedup_policy=insert`), supersede originals; **(2) conflict-resolution** (`CONFLICT_RESOLUTION_ENABLED`, tag-gated threshold 0.80) — among same-topic contradictory rows, LLM picks the current survivor and the stale ones are superseded in its favor via `forget(...replaced_by_id...)` (no synthesized text; decisions naming ids outside the cluster are rejected). Merge pass runs first. **Off by default many ways** (crontab line commented, both `*_ENABLED` false, no-op without LLM). Skips any cluster with a `keep='Y'` member; only ever soft-deletes (recoverable). **Cost-bounded**: ≤ `CONSOLIDATION_MAX_GROUPS` (default 25) small LLM calls per enabled pass, zero when no clusters exist. Actions + summary logged at WARN for audit/undo. Rollback: redeploy previous WAR (and re-comment the crontab line). |
| Learning memory Tier 4 phase 1 — contextual retrieval policy (`OwnSona.md` Tier 4; `SystemAnalysis.md` Part 5) | **code-complete** | **Procedure B**: `CURRENT_DB_VERSION` → **7** via `Migration007AddContextVector` (adds `context_vector vector(1536)` nullable + `context_count int`). Each memory learns a `context_vector` — a reward-weighted centroid of the query embeddings it was reinforced as helpful for. `reinforce` gains an optional `query`; on positive feedback the server embeds it (best-effort) and moves each reinforced memory's centroid toward it (`VectorFormat.blend` at `CONTEXT_ETA`=0.3, in-Java read/blend/write to avoid pgvector-operator version issues). `recall` adds `CONTEXT_RANK_WEIGHT`(0.5)·`context_match` to the rank blend (`context_match` = query↔centroid cosine, **0 when NULL** so behavior is unchanged until used). Embeddings-only (no generative call) → allowed on the reinforce path. The centroid is not in `SELECT_COLUMNS`/`MemoryRow` (only `context_count` is); referenced directly in `findRanked`. CLI `reinforce --query`. This is Part 5's learned selection policy scoped to memory selection. Rollback SQL: `ALTER TABLE memories DROP COLUMN context_vector, DROP COLUMN context_count;`. **Tier 4 phase 2 (relation/concept graph) is the next, separate phase.** |
| Learning memory Tier 4 phase 2 — relation/concept graph (multi-hop) (`OwnSona.md` Tier 4) | **code-complete** | **Procedure B**: `CURRENT_DB_VERSION` → **8** via `Migration008AddRelations` (new `memory_relations` table — `subject`/`predicate`/`object`/`source_memory_id` ON DELETE CASCADE — + `relations_extracted` flag on `memories`; table owned by the `ownsona` role, no extra GRANTs). Background `GraphExtractionJob` (Kiss Cron via `ExtractRelations.groovy`) uses the generative seam to extract triples once per memory; off by default (crontab commented, `GRAPH_EXTRACTION_ENABLED=false`, no-op without LLM), ≤`GRAPH_EXTRACTION_MAX_MEMORIES` (25) calls/run, zero once extracted. New read-only `query_relations` MCP tool + CLI `relations <entity> [--max-hops N]`: pure BFS over the graph (no LLM, hot-path-safe), undirected reachability, soft-deleted sources excluded, capped (`GRAPH_MAX_RELATIONS`=200, hops cap 5). Limitation: edited memories keep original triples (traversal still shows current text). Rollback SQL: `DROP TABLE memory_relations; ALTER TABLE memories DROP COLUMN relations_extracted;`. |
| `keep` protection flag + CLI curation (post-plan feature) | **deployed** (2026-06-04) | Procedure B: `CURRENT_DB_VERSION` bumped to **5** via `Migration005AddKeep` (adds `keep CHAR(1) NOT NULL DEFAULT 'U' CHECK (keep IN ('Y','N','U'))`). `keep='Y'` blocks update / forget / their batch variants for **every** client (service-layer `requireNotProtected` → `PROTECTED`); the flag itself is changed only via the CLI-only, secret-gated (`OwnsonaAdminSecret`), unadvertised `set_keep` tool. Extended the `ownsona` CLI with curation commands (`display` / `change` / `delete` / `enumerate`, id selectors, `-k` keep filter). |
